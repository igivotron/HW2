...
For a Brayton cycle, the optimum compressor temperature ratio is given by:
\begin{equation}
    \tau_c = \sqrt{\tau_\lambda}
\end{equation}
We can see that the optimum expression is very close to the one found for the turbojet, the only difference is the presence of the ram effect $\tau_r$ in the denominator. This is logical since the Brayton cycle doesn't take into account the ram effect while the turbojet does.\\
...
We have assumed until now that the engine was operating at design conditions. We want to analyse the performance of the turbojet when operating off-design conditions.\\
We consider a turbojet with a design inlet total temperature ratio, $\theta_{0,R} = 1.3$.\\
First we need to define the off-design inlet total temperature ratio:
\begin{equation}
    \theta_0 = \frac{T_{t,0}}{T_{SL}} = \frac{T_0}{T_{SL}} \tau_r = \frac{T_0}{T_{SL}} (1 + \frac{\gamma -1}{2} M_{0}^2) 
\end{equation}

We have $\theta_{0,R} = \frac{T_{t,0R}}{T_{SL}} = \frac{T_0}{T_{SL}}\tau_{r,R}$

\subsection{Effect of off-design on $\tau_c$ and $\tau_\lambda$}
First, we recall that at $\theta_0$ we have:
\begin{equation}
    \tau_c = 1 + \frac{T_{t,4}}{\theta_0 T_{SL}} ( 1 - \tau_{t, R})
\end{equation}

\begin{equation}
    \tau_\lambda = \frac{T_{t,4}}{T_{t,0}} = \frac{T_{t,4}}{\theta_0 T_{SL}}
\end{equation}

\subsubsection{Find $\tau_{t,R}$}
We need to find $\tau_{t,R}$ to compute $\tau_c$ at off-design conditions. We start from the equilibrium between the compressor and the turbine at design conditions \ref{eq:taut0}:

\begin{align*}
    \tau_{t,R} &= 1- \frac{\tau_{r,R}}{\tau_{\lambda, R}}(\tau_{c,R} -1) & \text{At design point we have that: } \tau_{r, R} = \theta_{0,R} \frac{T_{SL}}{T_0}\\
    \tau_{t,R} &= 1- \frac{\theta_{0,R} \frac{T_{SL}}{T_0}}{\tau_{\lambda, R}}(\tau_{c,R} -1) & \text{At design point: } \tau_{\lambda, R} = \frac{T_{t,4}}{T_{t,0R}} = \frac{T_{t,4}}{\theta_{0,R} T_{SL}}\\
\end{align*}

We consider that $\tau_{c,R} = \pi_{c,R}^{\frac{\gamma -1}{\gamma \eta_c}}$ with $\pi_{c,R}$ the compression ratio defined previously.

\subsubsection{Numerical application}
We have plotted $\tau_c$ and $\tau_\lambda$ as a function of the Mach number using the same parameters as in the previous part.
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/OD1.pdf}
    \caption{$\tau_c$ and $\tau_\lambda$ vs Mach Number for Turbojet}
    \label{fig:tauctaulambda}
\end{figure}
